...
export default function EditStaffProjectForm() {
  ...
  const isValidObjectIdString = (val) =>
  ...
  const handleInputChange = (e) => {
    if (
      e.target.name === "projectStartDate" &&
      formData.projectCompletedDate &&
      e.target.value > formData.projectCompletedDate
    ) {
      toast.error("Start date cannot be greater than the completion date.");
      return;
    } else if (
      e.target.name === "projectCompletedDate" &&
      formData.projectStartDate &&
      e.target.value < formData.projectStartDate
    ) {
    ...
    if (e.target.name === "projectCode") {
      ...
      if (val?.length > 14) {
        toast.error("Project Code must not be greater than 14 characters.");
        return;
      }
    }
  ...
  const getProjectById = async () => {
    try {
      ...
      // console.log("Response customer type ", response.data.result?.customerType)
      if (response.data.statusCode === 200) {
        const billingType = response.data.result.billingType;
        const isCnbProject = String(billingType || "").trim().toLowerCase() === "cnb";
        let normalizedCrew = response.data.result.crew;

        // CNB projects can have crew stored as string/csv; normalize to array so
        // existing "at least one crew" submit validation works correctly.
        if (isCnbProject && !Array.isArray(normalizedCrew)) {
          if (typeof normalizedCrew === "string") {
            const trimmed = normalizedCrew.trim();
            if (trimmed.startsWith("[") && trimmed.endsWith("]")) {
              try {
                normalizedCrew = JSON.parse(trimmed);
              } catch (e) {
                normalizedCrew = trimmed.split(",");
              }
            } else {
              normalizedCrew = trimmed.split(",");
            }
          } else if (normalizedCrew) {
            normalizedCrew = [normalizedCrew];
          } else {
            normalizedCrew = [];
          }
          normalizedCrew = normalizedCrew
        ...
        setInitialCrews(Array.isArray(normalizedCrew) ? normalizedCrew : []);
  ...
  function convertDateToMilliseconds(dateString) {
    ...
    // Return the time in milliseconds since January 1, 1970 UTC
    return date.getTime();
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData?.isProjectStarted && !formData?.projectCode) {
    ...
    try {
      ...
      if (formData.isProjectStarted) {
        formdata.append("projectCode", formData.projectCode);
      }
      ...
      // Avoid sending "null"/invalid jobType which causes backend ObjectId cast error
      const safeJobType =
        typeof formData.jobType === "string" ? formData.jobType.trim() : "";
      if (isValidObjectIdString(safeJobType)) {
        formdata.append("jobType", safeJobType);
      }
      ...
      if (formData.projectStartDate) {
        formdata.append(
          "projectStartDate",
          convertDateToMilliseconds(formData.projectStartDate)
        );
      }
      if (formData.projectCompletedDate) {
        formdata.append(
          "projectCompletedDate",
          convertDateToMilliseconds(formData.projectCompletedDate)
        );
      }
                      ...
                      <div
                        ...
                        className={`selected-crew ${
                          crew.status === "Active"
                            ? "bg-[#00613e]"
                            : "bg-danger"
                        } flex justify-center items-center rounded-full px-2 h-[32px] text-sm text-white`}
                      >
                        {crew.crewName}
                        <button
                          type="button"
