...
const handleFormInputChange = (e, formIndex, copyIndex) => {
  ...
  setFormData((prevState) => {
    ...
    if (name === "type") {
      ...
      updatedCopy.isTaxable = value === "Equipment Lump Sum" ? false : true;
    }
  ...
  const convertToBidedSchema = (formData) => {
    return formData.forms.map((form) => {
      ...
      return {
        _id : form._id,
        jobType: form.jobType, // actual project job name (string)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/proposal/edit-proposal/${id}`,
        formdata,
        {
          headers: headers,
        }
                        ...
                        form.copies.map((formData, copyIndex) => (
                          ...
                          >
                            ...
                            {formData.source === "Labor" && (
                              ...
                              <div className="form-group flex flex-col">
  <label htmlFor={`source-${formIndex}-${copyIndex}`}>
    Taxable *
  </label>
  <select
    name="isTaxable"
    ...
    className="border-b border-[grey] outline-none w-[180px]"
    value={formData.isTaxable ?? "false"}  // Default to "No"
    required
  >
