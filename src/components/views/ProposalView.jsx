...
export default function ProposalView() {
  ...
  const [bidCompletedDate, setBidCompletedDate] = useState("");
  ...
  const [categorizeCopies, setCategorizeCopies] = useState([]);
  const [jobTypeWise, setJobTypeWise] = useState([]);
  ...
  console.log("formDtaa621768238", formData);
  ...
  const getProposalById = async () => {
    try {
      ...
      // console.log("responseutdtr5667t87yuuihugyt",response);

      if (response.data.statusCode === 200) {
        let doc_name =
          response?.data?.result?.formData?.customerName?.toUpperCase() || "";
        ...
        setBidProjectId(response.data.result.formData.bidProjectId);
        setBidCompletedDate(response.data.result.formData.bidCompletedDate);

        let copies = [];
        for (let form of response.data.result.formData.forms) {
          copies = [...copies, ...form.copies];
        }
  ...
  useEffect(() => {
    ...
    const categorizedCopies = categorizeAndSummarize(copies);
    const catgCopies = formatCategorizedData(copies);
    setJobTypeWise(catgCopies);
    setCategorizeCopies(categorizedCopies);
  }, [selectedForms]);

  // console.log("Selected Forms", selectedForms, categorizeCopies);
  ...
  const saveInvoiceAs = async () => {
    try {
      ...
      if (!bidProjectId.trim()) {
        toast.error("Please enter project code.");
        return;
      }

      if (!bidCompletedDate.trim()) {
        toast.error("Please enter completed date.");
        return;
      }

      if (selectedCopies.length === 0) {
        toast.error("Please select a copy for invoice.");
        return;
      }
      ...
      formdata.append("bidCompletedDate", bidCompletedDate);
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/proposal/save-date-code/${id}`,
        formdata,
        {
          headers: headers,
        }
      );
      // console.log("Response Server : testing", response);
    } catch (error) {
  ...
  const downloadPdf = () => {
    const element = document.getElementById("content-to-pdf-proposal");
  ...
  const downloadInvoicePdf = () => {
    const element = document.getElementById("content-to-pdf-invoice");
    ...
    const options = {
      ...
      html2canvas: { scale: 2 },
      jsPDF: { unit: "in", format: "A4", orientation: "portrait" },
      // pagebreak: { mode: ["avoid-all"] }, // Ensures proper page breaks
    };
  ...
  function formatAddress(address) {
   ...
   function getCopiesAmounts(copies) {
    let taxAmount = 0;
    let subTotal = 0;
    let salesTax = 0;
    for (let type of copies) {
      subTotal = subTotal + Number.parseFloat(type.totalPrice);
      if (type.isTaxable) {
    ...
    if (formData?.customerType !== "Commercial") {
      salesTax = (taxPercent * taxAmount) / 100;
    }
    let grandTotal = subTotal + salesTax;
    return { taxAmount, subTotal, salesTax, grandTotal };
  }

  function categorizeAndSummarize(data) {
    const result = {};

    data.forEach((item) => {
      const { type, totalPrice, isTaxable, source } = item;
      ...
      result[type].totalPrice += totalPrice;
    });

    return result;
  }

  function formatCategorizedData(dataArray) {
    const grouped = {};

    dataArray.forEach((item) => {
      const key = `${item.isTaxable}_${item.type}_${item.source}`;
      ...
      grouped[key].totalPrice += item.totalPrice || 0;
    });
    ...
    return Object.values(grouped)
      .map((group) => ({
        name: `${group.type} ${
          group.source === "Labor" ? "LABOR" : handleInvoiceJobType(group.type)
        }`,
        source: group.source,
        isTaxable: group.isTaxable,
        type: group.type,
  ...
  console.log("selected Forms", formData.forms[0].jobAddress);
  ...
  return (
    <Layout>
      ...
      >
        ...
        >
          <div className="modal-dialog modal-dialog-centered" role="document">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <div className="form-group">
                  <label htmlFor="bidProjectId">Project Code</label>
                  <input
                    type="text"
                    className="form-control"
                    id="bidProjectId"
                    ...
                    value={bidProjectId}
                    onChange={(e) => {
                      setBidProjectId(e.target.value);
                    }}
                    name="bidProjectId"
                    maxLength={14}
                    required
                ...
                <div className="form-group">
                  <label htmlFor="bidCompletedDate">Completed Date</label>
                  <input
                    type="date"
                    className="form-control"
                    id="bidCompletedDate"
                    placeholder="e.g. Tuesday, January 4, 2025"
                    value={bidCompletedDate}
                    onChange={(e) => {
                      setBidCompletedDate(e.target.value);
                    }}
          ...
          <div className="card">
            ...
            <form onSubmit={handleSubmit}>
              <div className="card-body">
                ...
                <div className="form-group">
                  <label htmlFor="bidCompletedDate">Completion Date *</label>
                  <input
                    type="date"
                    className="form-control"
                    id="bidCompletedDate"
                ...
                {formData.forms.map((form, formIndex) => (
                  ...
                  >
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setSelectedForms((prev) =>
                            isChecked
                              ? [...prev, form]
                              : prev.filter((form) => form._id !== formId)
                          );
                        }}
                      />
                      <label
                        htmlFor={`selectForm-${formIndex}`}
                    ...
                    <div className="form-group">
                      <label htmlFor="bidCompletedDate">job Address *</label>
                      <input
                        className="form-control"
                        id="Job address"
              ...
              <div className="card-footer">
                {/* <button
                  ...
                  className="btn bg-[#00613e] text-white"
                >
                  {disableBtn ? "Loading..." : "Download PDF"}
                </button> */}
                <button
                ...
                <button
                  className={`bg-[#00613e] text-white py-1 px-6 md:mr-3 mr-0 ml-6`}
                  type="button"
                  data-toggle="modal"
                  data-target="#exampleModalCenter_invoice"
                  data-dismiss="modal"
                >
        ...
        {/* Downloaded Copy Proposal */}
        <div className="" style={{ display: "none" }}>
          <div className="relative mb-20" id="content-to-pdf-proposal">
            <div className="p-3">
              <div className="flex flex-row gap-3 justify-between">
                <div className="flex flex-col w-1/3 md:w-[280px]">
                  ...
                  <div className="p-0 capitalize">
                    <p className="text-xs break-words capitalize">
                      {formData.forms.map((id) => {
                        return <p>{id.jobAddress}</p>;
                      })}
                    </p>
                  </div>
            ...
            <div className="w-full mt-1 py-1 text-[13px] overflow-x-scroll p-3">
                    ...
                    <table className="w-full text-xs mt-6">
                      ...
                      <tbody>
                        {form &&
                          form.copies &&
                          form.copies.length > 0 &&
                          [...form.copies] // clone the array to avoid mutating original
                            .sort((a, b) => {
                              if (a.source === "Labor" && b.source !== "Labor")
                                return 1;
                              if (a.source !== "Labor" && b.source === "Labor")
                                return -1;
                              return 0;
                            ...
                            .map((item, idx) => (
                              <tr key={idx}>
                                ...
                                <td className="text-xs w-[400px] pr-2">
                                  ...
                                  {item.source === "Labor" && "LABOR"}
                                </td>
                                <td className="text-xs">{item?.measure}</td>
        ...
        >
          <div className="relative mb-20 text-xs" id="content-to-pdf-invoice">
            {/* Project Data */}
            <div className="flex flex-row gap-3 justify-between mt-1">
              ...
              <div className="flex flex-col w-1/3 md:w-[280px] text-end capitalize">
                ...
                <div className="p-0">
                  <p className="text-xs break-words">{formData.projectCode}</p>
                </div>
                <div className="p-0">
                  <p className="text-xs break-words">
                    {formData.salesOrderNumber}
                ...
                {bidCompletedDate && (
                  <div className="p-0">
                    <p className="text-xs break-words">
                      {convertToMMDDYYYY(bidCompletedDate)}
                    </p>
                  </div>
                )}
                <div className="p-0">
                ...
                <div className="p-0">
                  <p className="text-xs break-words">{formData.bidProjectId}</p>
                </div>
              </div>
            </div>
            {/* Description, Entries, Invoice */}
            <div className="w-full mt-1 py-1 text-[13px] overflow-x-scroll text-xs p-3">
              {selectedForms.length > 0 || selectedForms.length > 0 ? (
                selectedForms.map((form, index) => (
                  <div key={index} className="mb-0">
                    ...
                    <table className="w-[100%] text-xs mt-6 border-collapse">
                      <thead className=" border-collapse">
                        <tr className="gap-0 mx-0">
                          <th className="text-xs text-center">
                            <span className="relative -top-1.5">
                              DESCRIPTION
                            </span>
                        ...
                        {form &&
                          ...
                          [...form.copies] // clone the array to avoid mutating original
                            ...
                            .map((item, idx) => (
                              <tr key={idx}>
                                {/* <td className="text-xs">{item?.source}</td> */}
                                <td className="text-xs w-[400px] pr-2 break-words">
                                  {item?.reference?.toUpperCase()}{" "}
                                  {item.source === "Labor" && "LABOR"}
                                </td>
                                <td className="text-xs">{item?.measure}</td>
            ...
            <div className="mt-2 text-xs">
              {jobTypeWise &&
                jobTypeWise.map((item, index) => (
                  <div
                    className="flex justify-between mt-1 capitalize"
                    key={index}
                  >
                    <span>
            ...
            {
              <div className="text-xs w-full mt-5  py-2">
...
//                           ? type.includes("LABOR") || type.includes("OTHER")
//                             ? values.isTaxable
//                               ? "RT"
//                               : "RNT"
//                             : type.includes("LUMP SUM")
//                             ? `${
//                                 values.isTaxable ? "RT" : "RNT"
//                               } (SALES TAX PAID ON MATERIAL)`
//                             : "RT"
